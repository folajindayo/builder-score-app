...
function getCategoryReason(
  ...
  category: Exclude<BuilderCategory, null>,
  tokenPrice: number
): string {
  const rewardAmount =
    typeof user.reward_amount === 'string' ? parseFloat(user.reward_amount) : user.reward_amount;
  ...
  switch (category) {
    ...
    case 'featured':
      ...
      const verificationBonus =
        (user.profile.human_checkmark ? 150 : 0) + (user.profile.verified_nationality ? 75 : 0);
...
export function Leaderboard({ filters = {} }: LeaderboardProps) {
  ...
  const [allSponsorsAggregatedUsers, setAllSponsorsAggregatedUsers] = useState<
    UserWithEarningsBreakdown[]
  >([]);
  ...
  const [earningsRange, setEarningsRange] = useState<{ min: number | ''; max: number | '' }>({
    min: '',
    max: '',
  });
  ...
  const handleExportCSV = () => {
    ...
    const rows = data.users.map((user) => {
      ...
      return [
        ...
        user.profile.builder_score?.points
  ...
  useEffect(() => {
    ...
    JSON.stringify({
      sponsor_slug: filters.sponsor_slug,
  ...
  const fetchAllSponsorsLazy = async (
    filters: LeaderboardFilters,
    isLoadMore = false
    ...
    if (!isLoadMore) {
      ...
      tokenInfoMap = new Map(
        tokenPrices
          ...
          .map(({ slug, tokenInfo }) => [slug, tokenInfo!])
      );
    } else {
    ...
    const getBuilderKey = (user: LeaderboardUser): string => {
      ...
      const normalizedName = (user.profile.display_name || user.profile.name || '')
        .toLowerCase()
        .trim();
      if (normalizedName) {
    ...
    const existingEarningsBreakdownMap = new Map<
      string,
      Array<{
        sponsor: string;
        amount: number;
      ...
      if (result.status === 'fulfilled') {
        ...
        result.value.users.forEach((user) => {
          ...
          if (
            ...
            (existingUser.profile.builder_score?.points || 0)
          ) {
    ...
    if (filters.search) {
      ...
      filteredCombinedUsers = combinedUsers.filter((user) => {
        ...
        return (
          name.includes(searchLower) || bio.includes(searchLower) || wallet.includes(searchLower)
        );
      });
      console.log(
        `🔍 [All Sponsors Lazy] Search "${filters.search}": ${filteredCombinedUsers.length} results from ${combinedUsers.length} builders`
      );
    }

    // Display 30 builders at a time (or all if less than 30)
    const currentDisplayedCount = isLoadMore
      ? displayedCount + 30
      : Math.min(30, filteredCombinedUsers.length);
    ...
    console.log(
      `✅ [All Sponsors Lazy] Loaded ${combinedUsers.length} unique builders, ${filters.search ? `filtered to ${filteredCombinedUsers.length}` : ''}, displaying ${displayedUsers.length} of ${filteredCombinedUsers.length}`
    );
    return result;
  };
  ...
  const fetchAllSponsors = async (filters: LeaderboardFilters): Promise<LeaderboardResponse> => {
    ...
    const priceMap = new Map(tokenPrices.map(({ slug, price }) => [slug, price]));
    const tokenInfoMap = new Map(tokenPrices.map(({ slug, tokenInfo }) => [slug, tokenInfo]));

    // Fetch data from all sponsors
    // Fetch all pages sequentially for each sponsor (page=1, 2, 3, ...) with 100 per page
    console.log(
      '📡 [All Sponsors] Fetching leaderboard data from all sponsors (all pages, 100 per page, sequentially)...'
    );
    ...
    const fetchAllPagesForSponsor = async (slug: string): Promise<LeaderboardUser[]> => {
      ...
      while (hasMorePages) {
        try {
          ...
          if (response.users.length === 0) {
            ...
            console.log(
              `   ✓ ${slug} - Page ${currentPage}: ${response.users.length} builders (Total so far: ${allUsers.length})`
            );
      ...
      console.log(
        `   ✅ ${slug}: Completed - Fetched ${allUsers.length} builders across ${currentPage} page(s)`
      );
      return allUsers;
    };
    ...
    const earningsBreakdownMap = new Map<
      string,
      Array<{
        sponsor: string;
        amount: number;
      ...
      if (result.status === 'fulfilled') {
        ...
        result.value.users.forEach((user) => {
          ...
          if (!userMap.has(builderKey)) {
            ...
            console.log(
              `   🆕 New builder found: ${builderName} (Key: ${builderKey}, Talent ID: ${user.profile.talent_protocol_id || 'N/A'}, Sponsor ID: ${user.id})`
            );
          ...
          if (tokenInfo) {
            ...
            console.log(
              `   💵 Builder ${builderKey} (${builderName}, Sponsor ID: ${user.id}): ${rewardAmount} ${tokenInfo.symbol} × $${tokenPrice} = $${earningsUSD.toFixed(2)}`
            );
            ...
            console.log(
              `      Previous total: $${previousTotal.toFixed(2)} → New total: $${existingUser.totalEarningsUSD.toFixed(2)}`
            );
          }
    ...
    userMap.forEach((user, builderKey) => {
      ...
      const isExampleBuilder =
        builderName.includes('defidevrel') || builderName.includes('defidevrel.base.eth');
      ...
      if (isExampleBuilder) {
        ...
        console.log(
          `   Sponsors appeared in: ${user.sponsors.join(', ')} (${user.sponsors.length} sponsors)`
        );
        console.log(
          `   Total Earnings (totalEarningsUSD): $${(user.totalEarningsUSD || 0).toFixed(2)}`
        );
        console.log(`   Breakdown Sum Verification: $${breakdownSum.toFixed(2)}`);
        console.log(
          `   Match: ${Math.abs((user.totalEarningsUSD || 0) - breakdownSum) < 0.01 ? '✅ CORRECT' : '❌ MISMATCH'}`
        );
        console.log(`   Earnings Breakdown:`);
        user.earningsBreakdown.forEach((b, idx) => {
          const price = b.amount > 0 ? b.amountUSD / b.amount : 0;
          console.log(
            `      ${idx + 1}. ${b.sponsor}: ${b.amount} ${b.tokenSymbol} × $${price.toFixed(4)} = $${b.amountUSD.toFixed(2)}`
          );
        });
      }
    });
    ...
    if (!exampleBuilderFound) {
      ...
      userMap.forEach((user, builderKey) => {
        ...
        if (builderName.includes('defi') || builderName.includes('devrel')) {
          console.log(
            `   Similar match: ${user.profile.display_name || user.profile.name} (Key: ${builderKey}, Talent ID: ${user.profile.talent_protocol_id || 'N/A'}) - Sponsors: ${(user.sponsors || []).join(', ')}`
          );
        }
      });
    }
    ...
    if (exampleBuilderIndex !== -1) {
      ...
      console.log(
        `   Name: ${exampleBuilder.profile.display_name || exampleBuilder.profile.name || 'Anonymous'}`
      );
      console.log(
        `   Key: ${getBuilderKey(exampleBuilder)}, Talent ID: ${exampleBuilder.profile.talent_protocol_id || 'N/A'}`
      );
      console.log(
        `   Rank: #${exampleBuilderIndex + 1} (Position ${positionOnPage} on page ${pageNumber})`
      );
      console.log(
        `   Sponsors: ${(exampleBuilder.sponsors || []).length} (${(exampleBuilder.sponsors || []).join(', ')})`
      );
    ...
    combinedUsers.slice(0, 10).forEach((user, idx) => {
      ...
      console.log(
        `   ${marker} ${idx + 1}. ${user.profile.display_name || user.profile.name || 'Anonymous'} (Key: ${getBuilderKey(user)}, Talent ID: ${user.profile.talent_protocol_id || 'N/A'})`
      );
      console.log(
        `      Sponsors: ${(user.sponsors || []).length} (${(user.sponsors || []).join(', ')})`
      );
    ...
    console.log(
      `   Showing builders ${startIndex + 1} to ${Math.min(endIndex, combinedUsers.length)} of ${combinedUsers.length}`
    );
    ...
    console.log(
      `   Total earnings across all builders: $${combinedUsers.reduce((sum, u) => sum + (u.totalEarningsUSD || 0), 0).toFixed(2)}`
    );
      ...
      switch (sortBy) {
        ...
        case 'earnings':
          if (isAllSponsors) {
            ...
            const aAmt =
              typeof a.reward_amount === 'string' ? parseFloat(a.reward_amount) : a.reward_amount;
            const bAmt =
              typeof b.reward_amount === 'string' ? parseFloat(b.reward_amount) : b.reward_amount;
  ...
  if (!data || data.users.length === 0) {
      ...
      return (
        <div className="bg-white rounded-lg border border-gray-200 p-8">
          <div className="text-center">
            ...
            <p className="mt-4 text-gray-500 text-center text-sm font-medium">Search not found</p>
    ...
    return (
      <div className="bg-white rounded-xl border border-gray-200 p-8 shadow-sm">
        <p className="text-gray-500 text-center text-sm">No leaderboard data available</p>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-lg border border-gray-200">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  title={
                    autoRefresh ? `Auto-refresh every ${refreshInterval}s` : 'Enable auto-refresh'
                  }
                >
                  <svg
              ...
              <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-xl flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-blue-700">
                    {selectedForComparison.size} builder{selectedForComparison.size > 1 ? 's' : ''}{' '}
                      ...
                      ? formatNumber(
                          Math.round(
                            filteredUsers.reduce(
                              (sum, u) => sum + (u.profile.builder_score?.points || 0),
                              0
                        ...
                        ? `$${formatNumber(
                            Math.round(
                              filteredUsers.reduce((sum, u) => {
                                const amt =
                                  typeof u.reward_amount === 'string'
                                    ? parseFloat(u.reward_amount)
                                    : u.reward_amount;
        ...
        {!activeSearchQuery && topBuildersByCategory.length > 0 && (
          <div className="mb-6 bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-gray-200">
              <h3 className="text-lg font-semibold text-gray-900 mb-1">Top Builders by Category</h3>
              <p className="text-xs text-gray-500">Leading builders across different categories</p>
            </div>
                  ...
                  {topBuildersByCategory.map((item, idx) => {
                    ...
                    return (
                      ...
                      >
                        <td className="px-4 py-4">
                          <span
                            className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-full text-xs font-medium border ${categoryColors[category]}`}
                          >
                            <TrophyIcon category={category} className="w-3 h-3" />
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div className="min-w-0">
                              <div className="flex items-center gap-1.5">
                                ...
                                {isAllSponsors &&
                                  (user as UserWithEarningsBreakdown).sponsors &&
                                  (user as UserWithEarningsBreakdown).sponsors!.length > 1 && (
                                    <span
                                      className="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-purple-50 text-purple-700 border border-purple-200 flex-shrink-0"
                                      title="Appears in multiple sponsors"
                                    >
                                      Multi-Sponsor
                                    </span>
                                  )}
                              ...
                              {isAllSponsors &&
                                (user as UserWithEarningsBreakdown).sponsors &&
                                (user as UserWithEarningsBreakdown).sponsors!.length > 0 && (
                                  <div className="mt-1.5 flex flex-wrap gap-1">
                                    {(user as UserWithEarningsBreakdown).sponsors!.map(
                                      (sponsor) => (
                                        <span
                                          key={sponsor}
                            ...
                            if (isAllSponsors && userWithBreakdown.totalEarningsUSD !== undefined) {
                              return (
                                <div>
                                  ...
                                  <div className="text-xs text-gray-500">Total (All Sponsors)</div>
                                </div>
                              );
                        ...
                        <td className="px-4 py-4">
                          ...
                          {user.ranking_change !== 0 && (
                            <div
                              className={`text-xs ${user.ranking_change > 0 ? 'text-green-600' : 'text-red-600'}`}
                            >
        ...
        {viewMode === 'table' ? (
          <div className="overflow-x-auto rounded-xl" role="region" aria-label="Leaderboard table">
            <table
              ...
              role="table"
              aria-label="Builders leaderboard"
            >
                  ...
                  return (
                    ...
                    >
                      {visibleColumns.position && (
                        <td
                          className="px-4 py-4"
                          aria-label={`Position ${user.leaderboard_position || (data.pagination.current_page - 1) * 30 + idx + 1}`}
                        >
                          <div className="text-sm font-semibold text-gray-600">
                            {user.leaderboard_position ||
                              (data.pagination.current_page - 1) * 30 + idx + 1}
                          </div>
                        </td>
                      )}
                      {visibleColumns.name && (
                            ...
                            if (isAllSponsors && userWithBreakdown.totalEarningsUSD !== undefined) {
                              ...
                              return (
                                <div>
                                  ...
                                  {userWithBreakdown.earningsBreakdown &&
                                    userWithBreakdown.earningsBreakdown.length > 0 && (
                                      <div className="mt-1">
                                        <button
                                        ...
                                        {isExpanded && (
                                          <div className="mt-2 space-y-1.5 bg-gray-50 rounded-lg p-2 border border-gray-200">
                                            {userWithBreakdown.earningsBreakdown.map(
                                              (breakdown, idx) => {
                                                // Calculate running total up to this point
                                                const runningTotal = userWithBreakdown
                                                  .earningsBreakdown!.slice(0, idx + 1)
                                                  .reduce((sum, item) => sum + item.amountUSD, 0);

                                                return (
                                                  <div
                                                    key={idx}
                                                    className="flex items-center justify-between text-xs py-1 border-b border-gray-200 last:border-b-0"
                                                  >
                                                    <div className="flex-1">
                                                      ...
                                                      <div className="text-gray-500 mt-0.5">
                                                        {formatNumber(breakdown.amount)}{' '}
                                                        {breakdown.tokenSymbol} × $
                                                        {formatNumber(
                                                          breakdown.amountUSD / breakdown.amount ||
                                                            0
                                                        )}{' '}
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <button
                              ...
                              aria-label={
                                bookmarkedBuilders.has(user.id)
                                  ? 'Remove bookmark'
                                  : 'Bookmark this builder'
                              }
                            >
                              <svg
                                className="w-4 h-4"
                                fill={bookmarkedBuilders.has(user.id) ? 'currentColor' : 'none'}
                                stroke="currentColor"
                                viewBox="0 0 24 24"
                              >
                                <path
                            ...
                            <button
                              onClick={() =>
                                setEditingNote(editingNote === user.id ? null : user.id)
                              }
                              className={`p-1.5 rounded-lg transition-colors ${
                              ...
                              aria-label={
                                builderNotes[user.id]
                                  ? 'Edit note for this builder'
                                  : 'Add note for this builder'
                              }
                            >
                              <svg
                                className="w-4 h-4"
                    ...
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5 mb-1">
                          ...
                          {isAllSponsors &&
                            userWithBreakdown.sponsors &&
                            userWithBreakdown.sponsors.length > 1 && (
        ...
        {!filters.sponsor_slug &&
          (allSponsorsHasMore || (data && displayedCount < allSponsorsAggregatedUsers.length)) && (
