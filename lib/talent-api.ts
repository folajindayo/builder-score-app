import type { BuilderScore, BuilderProfile, SearchFilters, SearchResponse } from '@/types/talent';
...
export async function getBuilderScore(address: string): Promise<BuilderScore> {
  try {
    ...
    const response = await fetchAPI<{
      score: { points: number; rank_position: number; last_calculated_at: string; slug: string };
    }>(`/score?id=${encodeURIComponent(address)}`);
